...
class FeedGeorge extends Phaser.Scene {



 preload() {
    try {
        this.canvas.getContext("2d").imageSmoothingQuality = "high"
    } catch {}
    ...
    create() {
        ...
        this.george = this.physics.add.sprite(
          800 / 10,
          500 / 2,
          "george"
        );
        this.george.setScale(1.2)
        this.george.setSize(this.george.displayWidth - 300, this.george.displayHeight - 400)
      //  george.setSize(412/3, 605/3) 
      this.george.setFrame(1);
      this.george.setBounce(.75, 0.75);
      this.george.setCollideWorldBounds(true);
      
      
      this.georgeHead = this.physics.add.existing(this.add.zone(this.george.x, this.george.y, 100, 100))
      this.georgeHead.body.setAllowGravity(false)
      
      
      
      this.spat = this.physics.add.image(
          800 / 2,
          500 / 2,
          "spat"
        );
        this.spat.setScale(.5)
        this.spat.setBounce(0)
        this.spat.body.setAllowGravity(false)
        this.spatHead = this.physics.add.existing(this.add.zone(this.spat.x, this.spat.y, 140, 160))
        this.spatHead.body.setAllowGravity(false)
      
        this.physics.add.collider(this.george, this.spat);
      
      
        this.physics.add.overlap(this.spatHead, this.george, reflect, null, this);
        ...
        // this.physics.add.overlap(deleteZone, food, this.deleteFood, null, this);
        // this.physics.add.overlap(georgeHead, food, this.eatFood, null, this);
        // this.physics.add.overlap(spatHead, food, this.reflect, null, this)
        ...
        //   this.physics.add.collider(food, this.george);
       ...
       update() {
        ...
        //   georgeHead.body.y = george.body.y - 100;
          ...
          if (clicked){  
          this.spat.body.x = this.input.activePointer.x - this.spat.displayWidth + 15
          this.spat.body.y = this.input.activePointer.y - this.spat.displayHeight + 17}
      
      
          this.spatHead.body.x = this.spat.body.x + 1;
          this.spatHead.body.y = this.spat.body.y;
    ...
    deleteFood(deleteZone, food) {
        food.destroy()
    }
    
    eatFood(georgeHead, food) {
        food.destroy()
        foodEaten += 1
